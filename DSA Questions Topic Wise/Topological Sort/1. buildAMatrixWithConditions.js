...
// Output: []
// Explanation: From the first two conditions, 3 
// has to be below 1 but the third conditions needs 3 to be above 1 to be satisfied.
...
var buildMatrix = function(k, rowConditions, colConditions) {
    const buildTopologicalSort = (arr) => 
    {
        const graph = new Map();
        const indegree = new Array(k + 1).fill(0);
        let nodes, node;
        const stack = [];
        const result = new Array(k + 1);
        let index = 0;
        
        for(const [from, to] of arr) 
        {
            ++indegree[to];
            nodes = graph.get(from);
    ...
    const rows = buildTopologicalSort(rowConditions);
    ...
    const cols = buildTopologicalSort(colConditions);
    if(!cols)
    {
        return [];
    }
