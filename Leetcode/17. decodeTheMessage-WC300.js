...
// For example, given key = "happy boy" (actual key would have 
//     at least one instance of each letter in the alphabet), 
...
// It is obtained by taking the first appearance of each 
...
// Explanation: The diagram above shows the substitution table.
// It is obtained by taking the first appearance of each 
// letter in "eljuxhpwnyrdgtqkviszcfmabo".
...
var decodeMessage = function(key, message) {
    ...
    let decodedValuesSet =  new Set(key.split('').filter(str=>str!==" "))
    let decodedMsg = ""
    let i=0
    let setIterator = decodedValuesSet.values()
    
    
    for(let letter of setIterator)
    {
        decodedMap.set(letter,String.fromCharCode(i+97))
        i++
    }
    ...
    for(let letter of message)
    {
        ...
        {
            decodedMsg += decodedMap.get(letter)
        }
    }
