...
// where edges[i] indicates that there is a directed edge from 
...
var edgeScore = function(edges) {
    ...
    {
        if(edgeScoreArr[edges[i]]==undefined)
        {
            edgeScoreArr[edges[i]] = i 
        }
        else
        {
            edgeScoreArr[edges[i]] += i
    ...
    console.log(edgeScoreArr)
    
    for(let j=0; j<edgeScoreArr.length; j++)
    {
        if(localMax[1]<edgeScoreArr[j])
        {
            localMax[0] = j
            localMax[1] = edgeScoreArr[j]
        }
    }
