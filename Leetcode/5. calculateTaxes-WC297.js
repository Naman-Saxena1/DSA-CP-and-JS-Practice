...
var calculateTax = function(brackets, income) {
    let untaxedIncome = income;
    let taxPayed = 0
    let totalTaxToPay = 0
    let i = 0
    
    while(untaxedIncome>=0)  
    {
        
        if(brackets[i][0]<=income)
        {
            totalTaxToPay += (brackets[i][0]-taxPayed)*brackets[i][1]/100
            untaxedIncome = income - brackets[i][0]     
            taxPayed = brackets[i][0]                  
        }
        else
        {
            totalTaxToPay += (income-taxPayed)*brackets[i][1]/100
            untaxedIncome = 0
            taxPayed = income
        }
