...
// representing one unit of metal, paper and glass garbage respectively. 
...
// and must visit each house in order; however, 
...
var garbageCollection = function(garbage, travel) {
    let mflag = false, pflag = false, gflag = false;
    let minutes = 0
    ...
    for(let i=garbage.length-1; i>=0; i--)
    {
        ...
        if(mflag){minutes+=travel[i]}
        if(pflag){minutes+=travel[i]}
        if(gflag){minutes+=travel[i]}
        
        if(!mflag && garbage[i].includes("M"))
        {
            mflag = true
        }
        
        if(!pflag && garbage[i].includes("P"))
        {
            pflag = true
        }
        
        if(!gflag && garbage[i].includes("G"))
        {
            gflag = true
        }
    }
