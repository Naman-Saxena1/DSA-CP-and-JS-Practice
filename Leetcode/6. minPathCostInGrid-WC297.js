...
// Each possible move has a cost given by a 0-indexed 
// 2D array moveCost of size (m * n) x n, where moveCost[i][j] 
// is the cost of moving from a cell with value i to a cell in column j 
...
// plus the sum of costs of all the moves made. 
...
var minPathCost = function(grid, moveCost) {
    ...
    function move(row, col, cost) {
        ...
        {
            const addCost = moveCost[val][j];
            let newCostAnswer = move(row + 1, j, 0) + val + addCost
            ans = Math.min(ans, newCostAnswer);
        }
    ...
    let finalMinCost = Number.MAX_SAFE_INTEGER;
    
    for (let i = 0; i < cols; i++) 
    {
        finalMinCost = Math.min(finalMinCost, move(0, i, 0));
    }
    
    return finalMinCost;
};

console.log("Final: ",minPathCost([[5,3],[4,0],[2,1]], [[9,8],[1,5],[10,12],[18,6],[2,4],[14,3]]))